  ...
  var self = {
    walk: function walk(callback) {
      self.visitFeatures(features, function () {
        callback(!lifecycleError && featuresResult.isSuccessful());
      });
    },
    ...
    visitFeatures: function visitFeatures(features, callback) {
      ...
      self.broadcastEventAroundUserFunction(
        event,
        function (callback) {
          self.visitBeforeAllHooks(function() {
            if (lifecycleError) {
              callback();
            else {
              features.acceptVisitor(self, function () {
                self.visitAfterAllHooks(callback);
              });
            }
          });
        },
    ...
    visitBeforeAllHooks: function visitBeforeAllHooks(callback) {
      var beforeAllHooks = supportCodeLibrary.getBeforeAllHooks();
      self.visitLifecycleHooks(beforeAllHooks);
    },

    visitAfterAllHooks: function visitAfterAllHooks(callback) {
      var afterAllHooks = supportCodeLibrary.getAfterAllHooks();
      self.visitLifecycleHooks(afterAllHooks);
    },

    visitLifecycleHooks: function visitLifecycleHooks (hooks, callback) {
      var defaultTimeout = self.getDefaultTimeout();
      Cucumber.Util.asyncForEach(hooks, function (hook, callback) {
        if (lifecycleError) {
          callback();
        else {
          hook.invoke(defaultTimeout, function(error) {
            if (error) {
              error.message = "LifecycleHook error: "
              throw error;
            }
            callback();
          });
    ...
    executeLifecycleHook: function executeLifecycleHook(lifecycleHook, callback) {
      var defaultTimeout = self.getDefaultTimeout();
      lifecycleHook.invoke(defaultTimeout, function(error) {
        if (error) {
          featuresResult.
        }
      });
